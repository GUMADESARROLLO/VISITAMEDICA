...
class home_model extends CI_Model {
    ...
    public function listandoRutas() {
        ...
        //$query = $this->sqlsrv->fetchArray("SELECT * FROM vm_Mensuales_vtsTotales WHERE RUTA IN ('F03')", SQLSRV_FETCH_ASSOC);
        if (count($query)>0) {
    ...
    public function listandoData2($tipo, $ruta) {
        $i = 0;
        $json = array();
    ...
    public function detallesVentas3M($val, $tipo, $ruta) {
        $i = 0;
        $json = array();

        switch ($tipo) {
            case '1':
                $query = $this->sqlsrv->fetchArray('SELECT CCL AS CCL,NOMBRECL AS NOMBRECL,Cantidad AS Cantidad,Dia AS Dia,Venta AS Venta FROM vm_3M_vstCLA WHERE ARTICULO = '."'".$val."'".' AND RUTA = '."'".$ruta."'".';', SQLSRV_FETCH_ASSOC);
                if (count($query)>0) {
                    foreach ($query as $key) { 
                        $json[$i]['CCL'] = $key['CCL']; 
                ...
                break;
            case '2':
                $query = $this->sqlsrv->fetchArray('SELECT ARTICULO AS ARTICULO,DESCRIPCION AS DESCRIPCION,Cantidad AS Cantidad,Venta AS Venta,Dia AS Dia FROM vm_3M_vstCLA WHERE CCL = '."'".$val."'".' AND RUTA = '."'".$ruta."'".';', SQLSRV_FETCH_ASSOC);
                if (count($query)>0) {
                    foreach ($query as $key) { 
                        $json[$i]['ARTICULO'] = $key['ARTICULO']; 
                        $json[$i]['DESCRIPCION'] = $key['DESCRIPCION'];
